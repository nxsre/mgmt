...
func NewRedisClient() *redis.Client {
	...
	return redis.NewClient(&redis.Options{
		...
		IdleTimeout:        time.Duration(cfg.DefaultInt("redis::idle_timeout", 5000)) * time.Millisecond,
		IdleCheckFrequency: time.Duration(cfg.DefaultInt("redis::idle_check_frequency", 5000)) * time.Millisecond,
